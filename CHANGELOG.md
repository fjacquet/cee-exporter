...
- `evtx-readback` CI job. Downloads a `.evtx` generated by the new Linux
  ...
  OUT-06, which had been marked Unverified since v2 — and its first catch
  came before the job had even run in CI: evaluating go-evtx v0.7.1 as a
  further candidate bump, a file with three records and five empty field
  values wrote fine on Linux and read fine there under python-evtx, but
  failed on Windows under `Get-WinEvent` with `Object reference not set to
  an instance of an object` — no record returned, at file level. Deterministic
  over repeated runs; a file with the same record count but no empty values
  passed under v0.7.1, ruling out record count and isolating the cause to
  zero-length substitution values. Tracked upstream as
  [fjacquet/go-evtx#10](https://github.com/fjacquet/go-evtx/issues/10); this
  project stays on v0.7.0.
- `evtx-oracle` CI job and `tools/evtx-debug/verify_evtx.py`. Checks the same
  ...
  pre-v0.7.0 files Windows rejects, and it accepted the rejected v0.7.1 file
  above without complaint too — so it is a structural second opinion, not the
  Windows proof; only a Windows read-back job catches what it misses.
- `docs/windows-verification.md` section 5 — the dated manual Event Viewer
  protocol. It records two things CI cannot check: opening a saved log in
  Event Viewer's GUI, not run on 2026-08-10 since `winvm` is reached only over
  SSH with no interactive desktop session; and description rendering from a
  saved log, measured on 2026-08-10 not to work even with the event source
  confirmed registered in that same session — the live Application log
  rendered all three descriptions correctly in parallel, ruling out the
  registration trap the protocol otherwise warns about. Hypothesis: these
  records carry an empty `Channel`, so `LogName` resolves empty and Windows
  cannot bind the saved-log record to the registered provider. Confirming or
  fixing that would mean changing go-evtx, out of scope here.
...
- `github.com/fjacquet/go-evtx` v0.6.0 → v0.7.0. The library's BREAKING change
  ...
  only the writer API, so nothing here changed. v0.7.1 was evaluated as a
  further bump and rejected — see Added, above; the pin stays at v0.7.0.
- OUT-06 moves from **Unverified** to **Verified (partial)** in
  `docs/PROMISES.md`, `docs/requirements.md` and `docs/PRD.md`: the
  `Get-WinEvent` read-back path (file opens, record count, event ID set,
  `ToXml()`, `ObjectName`) is CI-gated on every push; description rendering
  from a saved log is not, and is recorded as a known limitation.
