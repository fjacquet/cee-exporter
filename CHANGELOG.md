...
- `evtx-readback` CI job. Downloads a `.evtx` generated by the new Linux
  ...
  v0.7.1 rather than staying back.
...
- `github.com/fjacquet/go-evtx` v0.6.0 → v0.7.1. v0.7.0's BREAKING change
  removes `Reader.ReadRecord()` and the `Record` struct; this project uses
  only the writer API, so nothing here changed. v0.7.1 tightens `<Provider>`
  rendering — an empty `ProviderName` now omits the `Name` attribute instead
  of emitting `Name=''` — which is what surfaced this project's own
  `-emit-test-events` defect (see Fixed, above) rather than a library
  regression. v0.7.1 is the pin this project stays on.
- OUT-06 moves from **Unverified** to **Verified** in `docs/PROMISES.md`,
  ...
  records enumerate, all twelve `EventData` fields carry correct values, and
  descriptions render. The first three of those are CI-gated on every push
  via `evtx-readback`; description rendering from a saved log is covered by
  the dated manual protocol in `docs/windows-verification.md`, since it
  needs a host with the event source registered.
