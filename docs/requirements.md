...
|----|-------------|--------|-------------|
| CEPA-01 | Listener completes the RegisterRequest handshake with a `<RegisterResponse>` carrying an identity CEE will accept | Delivered | `pkg/server/server_test.go::TestServeHTTP_RegisterRequest_ReturnsRegisterResponse` asserts the four attributes `CEndPoint::Init()` requires and a non-zero event filter; `TestServeHTTP_RegisterResponse_IsUTF16WhenAsked` pins the encoding mirroring. Both mutation-tested. **Corrected 2026-08-22 — this row said "HTTP 200 OK and strictly empty body" and that was the inverse of the truth.** An empty body fails with `Top node is not RegisterResponse`, no partner is registered, and CEE answers the array `0x16 CEPP_NOT_FOUND` while every observable stays green. The identity must also appear in CEE's compiled-in `CGuidStore` allowlist — see `[cepa]` in `config.toml.example`. Verified end to end against a live PowerStore on 2026-08-22. |
...
| CEPA-06 | Listener registers under an identity present in CEE's `CGuidStore` allowlist, configurable via `[cepa]` | Delivered | `pkg/server/register.go`. CEE refuses any other identity (`unknown or invalid GUID` / `GUID mismatch`) and then answers the array `0x16 CEPP_NOT_FOUND`, so this is the difference between receiving every event and receiving none. Defaults are deliberately values CEE refuses — see ADR-017. `TestServeHTTP_RegisterRequest_ReturnsRegisterResponse` covers the document; the identity itself is operator configuration, verified end to end against PowerStore `diabps01` on 2026-08-22. |
| CEPA-07 | Listener answers CEE's `<HeartBeatRequest />` with `hbStatus=0` so the partner is reported ONLINE | Delivered | `pkg/server/heartbeat.go`; `pkg/server/checkevent_test.go::TestServeHTTP_HeartBeatRequest_ReportsOnline`. Without it registration succeeds but the array receives `0x12 OFFLINE` and still publishes nothing. The `CEPP_SERVICE_*` values are read from an indexed pointer table in the CEE binary, not inferred from string order. |
| CEPA-08 | Listener parses Dell CEE's `<CheckEventRequest>` event delivery | Delivered | `pkg/parser/checkevent.go`; `pkg/parser/checkevent_test.go`, `pkg/server/checkevent_test.go`. This is a third dialect, distinct from the VCAPS `<CEEEvent>` shapes in CEPA-03/04 and from OneFS's `<CheckFileRequest>`. Event codes are a 21-bit mask; **one bit is measured directly, six more are corroborated by the independent OneFS table** (`TestEventTablesCorroborate`), the remaining fourteen are documented-only and marked provisional in the source. |
| CEPA-09 | Dispatcher transcodes each request body once | Delivered | `pkg/parser/classify.go::Classify`. Each `Is*` predicate decodes the whole body to read its root element, so dispatching through four of them and then parsing decoded the same payload five times — 62% of request time and 92% of allocations on a 1000-event batch, against a ~3 s CEPA deadline and a 64 MiB body cap. |
