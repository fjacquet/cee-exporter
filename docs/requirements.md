...
| DEPLOY-03 | `cee-exporter.exe install` registers the daemon with the Service Control Manager | Delivered | The `windows` CI job's "Service install/uninstall round-trip" step, confirmed in run [31315601780](https://github.com/fjacquet/cee-exporter/actions/runs/31315601780)'s raw log: `sc.exe query cee-exporter` immediately after `install` returned `STATE: 1 STOPPED` — a service that did not exist before this run genuinely exists in the SCM, not merely that the install command printed no error. Caveat: the step has no explicit `$LASTEXITCODE` check on the native `.exe` calls, so a silent install failure that still exits non-zero without a terminating PowerShell error would not by itself fail the step; this run's output was real success text, not a masked failure |
| DEPLOY-04 | `cee-exporter.exe uninstall` removes the daemon from the Service Control Manager | Delivered (partial) | Same step: `.\cee-exporter.exe uninstall` printed `Service "cee-exporter" uninstalled.` and exited 0. The job does not re-run `sc.exe query` after uninstall to independently confirm the service is actually gone from the SCM, so removal is confirmed only by the tool's own success message, not by re-querying the SCM afterward |
| DEPLOY-05 | Windows Service auto-restarts after an unexpected crash | **Unverified** | Nothing. No Windows CI runner. Recovery-action configuration is correct by inspection (`cmd/cee-exporter/service_windows.go`). |
...
| ID | Requirement | Status | Verified by |
|----|-------------|--------|-------------|
| EXT-01 | `github.com/fjacquet/go-evtx` exists as an independent Go module with its own repo, CI, README | Delivered (partial) | `go.mod`/`go.sum` (`github.com/fjacquet/go-evtx v0.6.0`). The module's own repository, CI, and README are external and not verifiable from this repo. |
