...
| ID | Requirement | Status | Verified by |
|----|-------------|--------|-------------|
| WIN-01 | Writes events to the Windows Application log via ReportEvent API | **Unverified** | Nothing. `pkg/evtx/writer_windows.go` has no `_test.go` counterpart. CI cross-compiles the Windows binary (`.github/workflows/ci.yml`) but never executes it — no Windows runner. |
| WIN-02 | Registers the "PowerStore-CEPA" event source on first start | **Unverified** | Nothing. Same reason as WIN-01. |
...
| DEPLOY-03 | `cee-exporter.exe install` registers the daemon with the Service Control Manager | **Unverified** | Nothing. No Windows CI runner. |
| DEPLOY-04 | `cee-exporter.exe uninstall` removes the daemon from the Service Control Manager | **Unverified** | Nothing. No Windows CI runner. |
