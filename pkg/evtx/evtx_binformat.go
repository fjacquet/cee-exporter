// evtx_binformat.go — pure binary format helpers for .evtx file construction.
//
// No build tag: these helpers are platform-agnostic (pure math, no OS calls).
// They are consumed by the BinaryEvtxWriter on non-Windows platforms and are
// testable on Linux CI without any cross-compilation.
//
